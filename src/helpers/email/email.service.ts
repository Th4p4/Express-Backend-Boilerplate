...
import { newAccountMail } from '../../config/constants';
...
export const sendVerificationEmail = async (
  ...
  const verificationEmailUrl = `${config.clientUrl}?modal=verify-email-token&token=${token}`;
  const text = `Hi ${name},
  ...
  const html = newAccountMail({
    appName: config.appName,
    copyrightDate: dateEarned.getFullYear(),
