...
export interface DocumentRes extends Document {
  createdAt: string;
  updatedAt: string;
}
    ...
    async function (filter: Record<string, any>, options: IOptions): Promise<QueryResult> {
      ...
      if (options.populate) {
        options.populate.split(',').forEach(populateOption => {
          const [path, selectedFields] = populateOption.split(';');
          let populateObject;

          if (selectedFields) {
            const selectFields = selectedFields.split(':').join(' ');
            if (path.includes('.')) {
              const [parentPath, childPath] = path.split('.');
              populateObject = {
                path: parentPath,
                populate: { path: childPath, select: selectFields },
              };
            } else {
              populateObject = { path, select: selectFields };
            }
          } else {
            populateObject = path.includes('.') ? { path } : path;
          }
